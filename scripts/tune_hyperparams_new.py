...
def load_dists(all_save_dir, data, split, model, joint_threshold):
    save_dir = "{}/{}_{}_{}_joint{}_top50cands_final_joint".format(
        all_save_dir, data, split, model, joint_threshold
    )
    ...
    biencoder_indices = np.load(
        os.path.join(save_dir, "biencoder_nns.npy"), allow_pickle=True
    )  # corresponds to biencoder_dists
    biencoder_dists = np.load(
        os.path.join(save_dir, "biencoder_dists.npy"), allow_pickle=True
    )
    if os.path.exists(os.path.join(save_dir, "biencoder_cand_scores.npy")):
        cand_dists = np.load(
            os.path.join(save_dir, "biencoder_cand_scores.npy"), allow_pickle=True
        )
    else:
        cand_dists = np.load(
            os.path.join(save_dir, "biencoder_cand_dists.npy"), allow_pickle=True
        )
    pred_mention_bounds = np.load(
        os.path.join(save_dir, "biencoder_mention_bounds.npy"), allow_pickle=True
    )
    if os.path.exists(os.path.join(save_dir, "biencoder_mention_scores.npy")):
        mention_dists = np.load(
            os.path.join(save_dir, "biencoder_mention_scores.npy"), allow_pickle=True
        )
    else:
        mention_dists = [
            biencoder_dists[i]
            - torch.log_softmax(torch.tensor(cand_dists[i]), 1).numpy()
            for i in range(len(biencoder_dists))
        ]
        # inverse sigmoid
...
def filter_repeat_overlaps(tokens, pred_triples, pred_scores):
    all_pred_entities_pruned = []
    all_pred_scores_pruned = []
    mention_masked_utterance = {
        triple[0]: np.zeros(len(tokens)) for triple in pred_triples
    }
    # ensure well-formed-ness, prune overlaps
    # greedily pick highest scoring, then prune all overlapping
    for idx, mb in enumerate(pred_triples):
...
def get_threshold_mask_and_sort(
    mention_dists,
    cand_dists,
    biencoder_dists,
    valid_cands_mask,
    threshold,
    top_mention_sort=True,
):
    """
    top_mention_sort:
    ...
    scores = torch.log_softmax(
        torch.tensor(cand_dists[valid_cands_mask]), 1
    ) + torch.sigmoid(torch.tensor(mention_scores)).log().unsqueeze(-1)
    if top_mention_sort:
...
if model_type == "wiki":
    model = "{0}_all_ents;all_mention_biencoder_all_avg_true_128_true_true_bert_large_qa_linear;15".format(
        model_type
    )
elif model_type == "finetuned_webqsp":
    model = "{0}_all_ents;all_mention_biencoder_all_avg_true_128_true_true_bert_large_qa_linear;18".format(
        model_type
    )
...
for data in ["nq", "WebQuestions", "triviaqa"]:
    ...
    for split in splits:
        (
            ...
            all_save_dir, data, split, model, "0.0" if model_type == "wiki" else "-inf"
        )
        new_examples = []
        num_correct = 0
        num_predicted = 0
        ...
        for i, example in enumerate(tqdm(examples)):
            ...
            valid_cands_mask = (biencoder_dists[i][:, 0] != -1) & (
                biencoder_dists[i][:, 0] == biencoder_dists[i][:, 0]
            )
            # get scores and masking/sorting by score
            (
                scores_mask,
                sorted_idxs,
                sorted_filtered_scores,
            ) = get_threshold_mask_and_sort(
                mention_dists[i],
                cand_dists[i],
                biencoder_dists[i],
                valid_cands_mask,
                threshold,
                top_mention_sort=(not get_topk_cands),
            )
            if get_topk_cands:
                # (filtered_examples, #cands, 2)
                ex_pred_mention_bounds = np.repeat(
                    np.expand_dims(pred_mention_bounds[i], axis=1),
                    biencoder_indices[i].shape[1],
                    axis=1,
                )
                # (filtered_examples, #cands,)
                ex_mention_dists = np.repeat(
                    np.expand_dims(mention_dists[i], axis=1),
                    biencoder_indices[i].shape[1],
                    axis=1,
                )
            ...
            threshold_entities = ex_biencoder_indices[valid_cands_mask][scores_mask][
                sorted_idxs
            ]  # (filtered_exs, #cands) / (filtered_cands,)
            threshold_mention_bounds = ex_pred_mention_bounds[valid_cands_mask][
                scores_mask
            ][
                sorted_idxs
            ]  # (filtered_exs, 2) / (filtered_cands, 2)
            threshold_cand_scores = ex_cand_dists[valid_cands_mask][scores_mask][
                sorted_idxs
            ]  # (filtered_exs, #cands) / (filtered_cands,)
            threshold_mention_scores = ex_mention_dists[valid_cands_mask][scores_mask][
                sorted_idxs
            ]  # (filtered_exs,) / (filtered_cands,)
            threshold_scores = (
                sorted_filtered_scores  # (filtered_exs, #cands) / (filtered_cands,)
            )
            ...
            for m in range(len(threshold_scores)):
                ...
                if len(threshold_entities[m].shape) > 0:
                    pred_triples.append(
                        [str(threshold_entities[m][0]), mb[0], mb[1] + 1]
                    )
                    ...
                    threshold_entities_translate[mention_text][
                        "cand_scores"
                    ] = threshold_cand_scores[m].tolist()
                    for id in threshold_entities[m]:
                        threshold_entities_translate[mention_text][
                            "candidate_entities"
                        ].append(id2title[str(id)])
                else:
                    pred_triples.append([str(threshold_entities[m]), mb[0], mb[1] + 1])
                    pred_scores.append(float(threshold_scores[m]))
                    threshold_entities_translate[mention_text][
                        "candidate_entities"
                    ] = id2title[str(threshold_entities[m])]
                    threshold_entities_translate[mention_text]["cand_scores"] = float(
                        threshold_cand_scores[m]
                    )
            new_ex = {
                "id": example["id"],
                "text": example["text"],
            ...
            if "gold_triples" in example:
                ...
                if get_topk_cands:
                    all_pred_entities_pruned, all_pred_scores_pruned = filter_repeats(
                        pred_triples, pred_scores
                    )
                    all_pred_entities_pruned = all_pred_entities_pruned[:topk]
                    all_pred_scores_pruned = all_pred_scores_pruned[:topk]
                else:
                    all_pred_entities_pruned, all_pred_scores_pruned = filter_overlaps(
                        example["tokens"], pred_triples, pred_scores
                    )
            else:
                all_pred_entities_pruned = pred_triples
                all_pred_scores_pruned = pred_scores
            ...
            new_ex["pred_triples_string"] = [
                [
                    id2title[triple[0]],
                    tokenizer.decode(example["tokens"][triple[1] : triple[2]]),
                ]
                for triple in all_pred_entities_pruned
            ]
            # get scores
            if "gold_triples" in example:
                ...
                num_overlap_weak, num_overlap_strong = entity_linking_tp_with_overlap(
                    gold_triples, new_ex["pred_triples"]
                )
        ...
        with open(
            os.path.join(
                "/checkpoint/belindali/entity_link/data/{}/saved_preds".format(data),
                save_file,
            ),
            "w",
        ) as wf:
