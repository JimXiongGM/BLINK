...
def _print_colorful_prediction(all_entity_preds, pred_triples, id2text, id2wikidata):
    ...
    for idx in sort_idxs:
        print(
            colored(
                all_entity_preds[0]["pred_tuples_string"][idx][1],
                "grey",
                HIGHLIGHTS[idx % len(HIGHLIGHTS)],
            )
        )
    ...
    for batch_num, batch_data in enumerate(dataloader):
        ...
        for b in range(len(batch_context)):
            ...
            if "label_id" in sample:
                # Get LABELS
                input_mention_idxs = batch_mention_idxs[b][
                    batch_mention_idx_masks[b]
                ].tolist()
                input_label_ids = batch_label_ids[b][batch_label_ids[b] != -1].tolist()
                assert len(input_label_ids) == len(input_mention_idxs)
                gold_mention_bounds = [
                    sample["text"][ment[0] - 10 : ment[0]]
                    + "["
                    + sample["text"][ment[0] : ment[1]]
                    + "]"
                    + sample["text"][ment[1] : ment[1] + 10]
                ...
                for my_input_start in range(len(gold_input)):
                    if (
                        gold_input[my_input_start] == input_context[0]
                        and gold_input[
                            my_input_start : my_input_start + len(input_context)
                        ]
                        == input_context
                    ):
                        break
                ...
                pred_triples = [
                    (
                        triple[0],
                        triple[1] + my_input_start,
                        triple[2] + my_input_start,
                    )
                ...
                gold_triples = [
                    (
                        ...
                        sample["tokenized_mention_idxs"][j][0],
                        sample["tokenized_mention_idxs"][j][1],
                    )
                ...
                pred_input_window_triples = [
                    (
                        str(all_pred_entities_pruned[j]),
                        int(e_mention_bounds_pruned[j][0]),
                        int(e_mention_bounds_pruned[j][1]),
                    )
                    for j in range(len(all_pred_entities_pruned))
                ]
                gold_input_window_triples = [
                    (
                        ...
                        input_mention_idxs[j][0],
                        input_mention_idxs[j][1] + 1,
                    )
                    for j in range(len(input_label_ids))
                ]
                (
                    num_overlap_weak_window,
                    num_overlap_strong_window,
                ...
                entity_results.update(
                    {
                        "pred_tuples_string": [
                            [
                                id2title[triple[0]],
                                tokenizer.decode(
                                    sample["tokenized_text_ids"][triple[1] : triple[2]]
                                ),
                            ]
                            for triple in pred_triples
                        ],
                ...
                if errors_f is not None and (
                    num_overlap_weak != len(gold_triples)
                    or num_overlap_weak != len(pred_triples)
                ):
...
def run(
    ...
    if (
        not test_data
        and not getattr(args, "test_mentions", None)
        and not getattr(args, "interactive", None)
    ):
        msg = (
    ...
    if getattr(args, "save_preds_dir", None) is not None and not os.path.exists(
        args.save_preds_dir
    ):
    ...
    if args.interactive:
        while not stopping_condition:
            ...
            while action == "c":
                ...
                _print_colorful_text(
                    all_entity_preds[0]["tokens"], tokenizer, pred_triples
                )
                ...
                if action == "c":
                    ...
                    while True:
                        threshold = input(
                            "New threshold (increase for less cands, decrease for more cands): "
                        )
                        try:
                            ...
                            print(
                                "Error! Expected float, got {}. Try again.".format(
                                    threshold
                                )
                            )
