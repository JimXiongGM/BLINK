...
def select_field_with_padding(data, key1, key2=None, pad_idx=-1):
    ...
    for i, entry in enumerate(selected_list):
        # pad to max len
        pad_list = [1 for _ in range(len(entry))] + [
            0 for _ in range(max_len - len(entry))
        ]
...
def get_context_representation_multiple_mentions_left_right(
    ...
    if (
        len(all_mentions[0]) == 0
        and len(all_context_lefts[0]) == 0
        and len(all_context_rights[0]) == 0
...
def get_context_representation_multiple_mentions_idxs(
    ...
    sort_tuples = [
        [i[0], i[1]]
        for i in sorted(enumerate(mention_idxs), key=lambda x: (x[1][0], x[1][1]))
    ]
    if [tup[1] for tup in sort_tuples] != mention_idxs:
        orig_idx_to_sort_idx = {itm[0]: i for i, itm in enumerate(sort_tuples)}
        assert [tup[1] for tup in sort_tuples] == sort_mentions(
            mention_idxs, orig_idx_to_sort_idx
        )
    ...
    all_mention_tokens = input_ids[
        all_mention_spans_range[0] : all_mention_spans_range[1]
    ]
    ...
    input_ids_window = (
        context_left[-left_quota:] + all_mention_tokens + context_right[:right_quota]
    )
    ...
    if len(input_ids) <= max_seq_length - 2:
        ...
        if cut_from_left > 0:
            ...
            for c in range(len(mention_idxs)):
                mention_idxs[c] = [
                    mention_idxs[c][0] - cut_from_left,
                    mention_idxs[c][1] - cut_from_left,
                ]
...
def process_mention_data(
    ...
    """
    extra_ret_values = {}
    if saved_context_dir is not None and os.path.exists(
        os.path.join(saved_context_dir, "tensor_tuple.pt")
    ):
        data = torch.load(os.path.join(saved_context_dir, "data.pt"))
        tensor_data_tuple = torch.load(
            os.path.join(saved_context_dir, "tensor_tuple.pt")
        )
    ...
    for idx, sample in enumerate(iter_):
        assert (
            not add_mention_bounds
        ), "Adding mention bounds, but we have multiple entities per example"
        ...
        if candidate_token_ids is not None:
            token_ids = [
                [candidate_token_ids[label_id].tolist()] for label_id in label_ids
            ]
            label_tokens = {
                "tokens": "",
                "ids": token_ids,
        ...
        if isinstance(sample["label_id"], list):
            ...
            if len(sample["label_id"]) > len(context_tokens["mention_idxs"]):
                sample["label_id"] = sample["label_id"][
                    : len(context_tokens["mention_idxs"])
                ]
            label_idx = [int(id) for id in sample["label_id"]]
        else:
            assert isinstance(sample["label_id"], int) or isinstance(
                sample["label_id"], str
            )
    ...
    if isinstance(processed_samples[0]["context"]["mention_idxs"][0], int):
        mention_idx_vecs = torch.tensor(
            select_field(processed_samples, "context", "mention_idxs"),
            dtype=torch.long,
        ).unsqueeze(1)
        mention_idx_mask = torch.ones(
            mention_idx_vecs.size(0), dtype=torch.bool
        ).unsqueeze(-1)
        if logger:
        ...
        mention_idx_vecs, mention_idx_mask = select_field_with_padding(
            ...
            "context",
            "mention_idxs",
            pad_idx=[0, 1],  # ensure is a well-formed span
        )
        # (bs, max_num_spans, 2)
        ...
        if not params["freeze_cand_enc"]:
            cand_vecs, cand_mask = select_field_with_padding(
                ...
                "label",
                "ids",
                pad_idx=[[0 for _ in range(max_cand_length)]],
            )
        ...
        label_idx_vecs, label_idx_mask = select_field_with_padding(
            processed_samples, "label_idx", pad_idx=-1
        )
        # (bs, max_num_spans)
    ...
    # save data
    if saved_context_dir is not None and not os.path.exists(
        os.path.join(saved_context_dir, "tensor_tuple.pt")
    ):
        os.makedirs(saved_context_dir, exist_ok=True)
        torch.save(data, os.path.join(saved_context_dir, "data.pt"))
        torch.save(
            tensor_data_tuple, os.path.join(saved_context_dir, "tensor_tuple.pt")
        )
    return data, tensor_data_tuple, extra_ret_values
